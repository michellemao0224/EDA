...
Apart from other occupations, professional occupies a large part of the count. Why loan providers prefer to lend the loan to professional? The reason behind is they have stable and high income compared to others whilst they work on more techniqual job, which show the strong debt paying ability.
...
The interest and fees follow the same pattern as the loan offering amount in the begining but different in the end. Interest and fees slightly increased from the end of 2006 to 2007. They also have been affected by the subprime mortgage crisis from 2008 to 2009 and had a significantly drop.
From 2010 to 2011, the interest and fees kept growing, but had a huge jump from 2012 to 2014. The decrease in loan interest rates in 2012 was affected by the bank start to provide more loan to borrowers. Loan providers lost a lot of their financial market in US.
...
b1 <- ggplot(aes(x = BorrowerAPR * 100, y = CurrentDelinquencies / 100), 
...
b2 <- ggplot(aes(x = BorrowerRate * 100, y = CurrentDelinquencies / 100), 
       data = subset(loan, !is.na(CurrentDelinquencies))) +
  geom_point(alpha = 1/10, position = position_jitter(h = 0), color = I('#D02090'), size = 0.8) +
  xlab('Current Delinquencies')  +
  ylab('Borrower Rate')

b3 <- ggplot(aes(x = LenderYield * 100, y = CurrentDelinquencies / 100), 
       data = subset(loan, !is.na(CurrentDelinquencies))) +
  geom_point(alpha = 1/10, position = position_jitter(h = 0), color = I('#D02090'), size = 0.8) +
  xlab('Current Delinquencies')  +
  ylab('Lender Yield') 
...
ggplot(aes(x = DebtToIncomeRatio * 10, y = CurrentDelinquencies / 100), 
       data = subset(loan, !is.na(CurrentDelinquencies))) +
...
ggplot(aes(x = IncomeRange, y = mean_BorrowerAPR),
...
This plot shows the customer quality from two aspects: average loan original amount and income range. 
...
This time I want to know how different between two similar prosper ratings. So I compared each pair of prosper ratings, such as: AA and A, A and B, etc. We can see the prosper rating ratio increases from 0.65 to 0.95. Especially, there is a big gap from AA/A to A/B.

In terms of the variety of income range, A/B and D/E ratio go up with the income range. B/C ratio goes up with the income range but significantly drop in the end. C/D and E/HR ratio go down with the income range. AA/A ratio almost stays the same.

Based on this diagram, we can find the perfect borrower APR for each borrower with differet income and prosper rating. This is also important for loan providers to measure their profitability.
...
ggplot(aes(x = ProsperRating..Alpha., y = mean_CurrentDelinquencies * 10),
